...
bool CanShoot(const marvin::Map& map, const marvin::Player& bot_player, const marvin::Player& target) {
  if (bot_player.position.DistanceSq(target.position) > 60 * 60)
    return false;
  if (map.GetTileId(bot_player.position) == marvin::kSafeTileId)
    return false;
...
public:
  ...
  behavior::ExecuteResult Execute(behavior::ExecuteContext& ctx) {
    ...
    CastResult result = RayCast(game.GetMap(), game.GetPosition(), Normalize(to_target), to_target.Length());

    return result.hit ? behavior::ExecuteResult::Failure : behavior::ExecuteResult::Success;
  }
...
public:
  behavior::ExecuteResult Execute(behavior::ExecuteContext& ctx) {
    ...
    Vector2f seek_position =
        CalculateShot(game.GetPosition(), target_pos, bot_player.velocity, target.velocity, proj_speed);
...
private:
  bool ShouldActivate(behavior::ExecuteContext& ctx, const Player& target) {
    ...
    float dot = game.GetPlayer().GetHeading().Dot(Normalize(target.position - position));
...
public:
  behavior::ExecuteResult Execute(behavior::ExecuteContext& ctx) {
    ...
    float distance_min = hover_distance - (aggression * (hover_distance - aggression_min));
    float distance_max = hover_distance * 1.2f;

    MapCoord spawn = ctx.bot->GetGame().GetSettings().SpawnSettings[0].GetCoord();

    if (Vector2f(spawn.x, spawn.y).DistanceSq(ctx.bot->GetGame().GetPosition()) < 35.0f * 35.0f) {
      distance_min = 0.0f;
      distance_max = 0.0f;
    }
...
public:
  behavior::ExecuteResult Execute(behavior::ExecuteContext& ctx) {
    ...
    auto to = ctx.blackboard.ValueOr<const Player*>("target_player", nullptr)->position;

    Path path = CreatePath(ctx, "path", from, to, game.GetShipSettings().GetRadius());
...
Bot::Bot(std::unique_ptr<GameProxy> game) : game_(std::move(game)), steering_(this) {
  ...
  auto target_in_los = std::make_unique<InLineOfSightNode>([](marvin::behavior::ExecuteContext& ctx) {
  ...
  auto invert_in_region = std::make_unique<behavior::InvertNode>(in_spawn_region.get());
  auto warp_node = std::make_unique<WarpNode>();
  auto warp_to_center = std::make_unique<behavior::SequenceNode>(invert_in_region.get(), warp_node.get());
...
void Bot::Steer() {
  ...
  if (has_force) {
    ...
    float force_percent = force.Length() / (GetGame().GetShipStatus().speed / 16.0f / 16.0f);
    RenderLine(center, center + (force_direction * 100 * force_percent), RGB(255, 255, 0));
  }
